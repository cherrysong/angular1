...
define(
    ...
    OperateMap.prototype = {
            ...
            _this.options = {
                ...
                mapStyle : {
                    ...
                    circle : {
                        stroke : 'rgba(0,70,156,1)',
                        r : 24,
                        xinshouFill:'#f96f04',//2016.5.16修改热力图水滴颜色
                        yijieFill: '#679d3d'
                    },
                    textStyle : {
                        fontSize : 92,
        ...
        initial : function (options) {
            ...
            // 设置地图的外观
            _this.setMapStyle();
            if(localStorage.getItem('fyjb')!=='0'){
...
/*
                ...
                if($('#xinjiang-group').length>0){
                    $('#mapid-480000-outline,#mapid-520000-outline,#mapid-xinjiangGroup-outline').attr({
                        stroke:'none',
        ...
        setMapStyle : function () {
            ...
            // 按哪组数排序（默认新收）
            _this.isXinshou = _this.opts.isXinshou;
...
//            console.log('initialAreaMap')
            $.each(_this.areaMapDataSource,function (index, value) {
                ...
                if ($('#' + _this.mapidPrefix + value.mapId + '-circle').length > 0) {
                    ...
                    // 设置circle的style
                    .attr({
                        fill : _this.mapStyle.circle.xinshouFill,
                    ...
                    var color = _this.mapStyle.circle.xinshouFill;
                    var waterOptions = {
                        x : 0, // 水滴的位置x
                        y : 0, // 水滴的位置y
                        fill : color, // 水滴的填充颜色
                   ...
                   _this.areaMapWater[_this.mapidPrefix + value.mapId] = _this.drawWater(waterSnap, waterOptions,1).water; // 会生成一个对象 例如：
                   _this.waterText[_this.mapidPrefix + value.mapId] = _this.drawWater(waterSnap, waterOptions,1).text;
                   
                       
                   /* _this.waterText[_this.mapidPrefix + value.mapId].attr({
                        display:'none'
                    })    
                   ...
                   /* if(value.mapId=='120000'||value.mapId=='320000'||value.mapId=='190000'){
                    ...
                    _this.areaDataText [_this.mapidPrefix + value.mapId] = _this.snap.el('text',{
                        class:'num-xinshou-'+value.mapId,
                        text:_this.areaMapData[_this.mapidPrefix + value.mapId].ajs,
                        x:_this.areaMapCircle[_this.mapidPrefix + value.mapId].attr('cx'),
            ...
            //
            _this.setWaterText(_this.areaMapDataSource);
        },
        ...
        creatOperateSvg : function (id, points) {
           ...
           // console.log('ratio'+_this.mapRatio)
            if(_this.mapRatio*2>1){
                 var  width=33*_this.mapRatio*6;
                 var  height=54*_this.mapRatio*6;
        ...
        // 画水滴
        drawWater : function (snap, options,index) { // snap 为svg对象。options 为参数设置
            var _this = this;
            var snap = snap; 
            var opts = $.fn.extend(true, {}, {
                ...
                x : 20, // 水滴的位置x
                y : 30, // 水滴的位置x
                scale : _this.mapRatio*18,   //  因为水滴还是太小了，所以在比率的基数上乘以2  
                data : {
                    mapId : '18000'
                }, // 水滴保存的数据
                fill : '#fff722', // 水滴的填充颜色
           ...
           //  水滴形状
            var water = snap.paper.el('path', {
                ...
                fill : opts.fill,
                id : 'water' + opts.id,
                transform : 'scale(' + _this.mapRatio*10 + ',' + _this.mapRatio*10 + ',32,54)',
                cursor : 'auto',
                'pointer-events' : 'auto',
                'display':'none'
            });
            //气泡中的数字
            var text = snap.paper.text(opts.x+4,opts.y+14,index+1).attr({
                transform : 'scale(' + _this.mapRatio*10 + ',' + _this.mapRatio*10 + ',32,54)',
                fontSize:14+"px",
                fill:'#000',
                'display':'none'
           ...
           // 放到g标签中，整体移动
            _this['waterGroup' + opts.id] = snap.g(water,text);
        ...
        creatNamText : function () {
            ...
            /*
             * 20160316 修改完之后再将这段代码放开并提交
            */
           /* if(changeAreamapTextPosition['map_'+mapId](_this)){
                changeAreamapTextPosition['map_'+mapId](_this);
            }*/
        },
        ...
        addAreaMapInteraction : function () {
            ...
            $.each(_this.areaMap, function (name, value) {
                ...
                value.hover(function (event) {
                    if (_this.isShowTooltip) {
                        ...
                        tooltipContent1Count.text(this.data('data').ajs);
                        var tbText = this.data('data').tb;
                        var tbClass="";
                        if(tbText>0){
                            tbClass = 'fd-td-color-up';
                        }else{
                            tbClass = 'fd-td-color-down';
                        }
                        //同比处理
                        tooltipContent2Count.removeClass("fd-td-color-up").removeClass("fd-td-color-down").addClass(tbClass);
                        tooltipContent2Count.text(global.handleTb(tbText));
                        //判断是否超出了屏幕的宽度和高度
                    ...
                    if (value.data('data').mapId == '520000'
                         || value.data('data').mapId == '480000'
                             || value.data('data').mapId == 'xinjiangGroup') {
                        value.attr({
                            opacity : 0.001
                        });
                    }
                ...
                function () {
                    ...
                    if (_this.isXinshou) {
                        areaCount = value.data('data').ajs;

                    } else {
                        areaCount = value.data('data').tb;
            ...
            if (_this.snap.select('#xinjiang-group')) {
                _this.snap.select('#xinjiang-group').hover(function () {
                    ...
                    var areaCount = 0;
                    if (_this.isXinshou) {
//                        console.log(_this.snap.select('#mapid-xinjiangGroup-outline').data('data'));
                        areaCount = _this.snap.select('#mapid-xinjiangGroup-outline').data('data').ajs;
                    } else {
                        areaCount = _this.snap.select('#mapid-xinjiangGroup-outline').data('data').tb;
                    ...
                    if (areaCount < _this.areaMapMinValue|| areaCount > _this.areaMapMaxValue) {
                        ...
                        _this.snap.select('#xinjiang-outline').attr({
                            fill : _this.snap.select('#mapid-xinjiangGroup-outline').data('data').fillColor
                        });
                    }
                });
            }
        ...
        updateData : function (dataSource, dataRange) {
            ...
            $.each(_this.areaMapDataSource, function (index, value) {
                ...
                _this.areaMapData[_this.mapidPrefix + value.mapId].name = $.trim(value.fyjc);
                _this.areaMapData[_this.mapidPrefix + value.mapId].ajs = Number(value.ajs);
                _this.areaMapData[_this.mapidPrefix + value.mapId].tb = Number(value.tb);
        ...
        setAreaMapDataRange : function () {
            ...
            $.each(_this.areaMapData, function (name, value) {
                _this.isXinshou ? _this.rangeArray.push(value.ajs)
                 : _this.rangeArray.push(value.tb);
            ...
            $.each(_this.areaMapData,function (name, value) {
                ...
                $.each(
                    ...
                    if (areaCount < value || areaCount == value) {
                        ...
                        //增加这个判断，是因为热力图的新疆区域用新疆和兵团的值的和xinjiangGroup，所以当拖动值域选择时，应该用xinjiangGroup的fillColor,520000和480000的颜色要设为无效，2016.7.12
                        if(_value.mapId=='520000'||_value.mapId=='480000'){
                            _value.fillColor=_this.opts.setGray;
                        }
                        return false; // 不再循环
                    }
                });
        ...
        setAreaMapFillColor : function () {
            ...
            $.each(_this.areaMap, function (name, value) {
                ...
                if (_this.isXinshou) {
                    ...
                    if(_this.areaMapWater[name]){
                        ...
                        _this.areaMapWater[name].attr({
                            stroke: _this.mapStyle.circle.xinshouFill,
                            fill: _this.mapStyle.circle.xinshouFill
                        });
                    }
                    ...
                    if(_this.areaMapWater[name]){
                        //设置水滴的颜色
                        _this.areaMapWater[name].attr({
                            stroke: _this.mapStyle.circle.yijieFill,
                            fill: _this.mapStyle.circle.yijieFill
                        });
                    }
                }
                ...
                if (areaCount < _this.areaMapMinValue || areaCount > _this.areaMapMaxValue) {
                    ...
                    // 判断是不是新疆这块区域
                    if (value.data('data').mapId == '520000' || value.data('data').mapId == '480000'|| value.data('data').mapId == 'xinjiangGroup') {
                        value.attr({
                ...
                if (_this.isSeventArea) {
                    ...
                    $.each(_this.areaMapCircle,function (name, value) {
                        ...
                        _this.snap.paper.el('line', {
                            ...
                            _this.snap.paper.el(
                                'text', {text : '案件数：' + value.data('data').ajs,
        ...
        updateDataText : function () {
            ...
            if (_this.isSeventArea) {
                $.each(_this.areaMap,function (name, value) {
                    var value = value;
                    //  更新数据
                    if ($(_this.snap.select('#' + name + 'ajs')).length) {
                        _this.snap.select('#' + name + 'ajs').attr({
                            text : '数量：' + value.data('data').ajs
                        });
                    }
                    if ($(_this.snap.select('#' + name + 'tb')).length) {
                        _this.snap  .select('#' + name + 'tb')
                        .attr({
                            text : '同比：' + value.data('data').tb
                        });
                    }
                });
            ...
            //  其他区域
                ...
                $.each(_this.areaMapCircle,function (name, value) {
                    ...
                    if(_this.isXinshou){
                        ...
                        if(parseInt(_this.areaMapData[_name].ajs) == parseInt(_this.areaDataText[_name].attr('text'))) {
                            return;
                        }
                        var text = _this.areaDataText[_name].clone().attr({
                            text: _this.areaMapData[_name].ajs
                        });
                   ...
                   //  如果是已结，显示已结的数据和已结的颜色
                        ...
                        if(parseInt(_this.areaMapData[_name].tb) == parseInt(_this.areaDataText[_name].attr('text'))) {
                            return;
                        }
                        var text = _this.areaDataText[_name].clone().attr({
                            text:_this.areaMapData[_name].tb
                        });
                        _this.areaDataText[_name].remove();
                        _this.areaDataText[_name] = text;
                    }
        ...
        hideDataText : function () {
            ...
            if (_this.isSeventArea) {
                $.each(_this.areaMapCircle,function (name, value) {
                    ...
                    // 删除新收文字
                    if (_this.snap.select('#' + name + 'ajs')) {
                        _this.snap.select('#' + name + 'ajs').remove();
                    }
                    // 删除已结文字
                    if (_this.snap.select('#' + name + 'tb')) {
                        _this.snap.select('#' + name + 'tb').remove();
                    }
                });
            }else {
        ...
        creatTipBox : function () {
            ...
            $(
                '<div id='
                 ...
                 + '<p  style="white-space:nowrap;"><span class="fd-name-01">案件数</span>：<span  class="fd-name-count-01">0</span></p>'
                 + '<p  style="white-space:nowrap;"><span class="fd-name-02">同比</span>：<span  class="fd-name-count-02">0</span></p>'
        ...
        setWaterText:function(data){
            var _this=this;
            $.each(_this.areaMapWater,function(index,value){
                value.attr({
                        display : 'none'
                });
            });
            $.each(_this.waterText,function(index,value){
                value.attr({
                        display : 'none'
                });
            })
            ...
            $.each(data,function(index,value){
                if(n<count){
                    if(_this.areaMapWater[_this.mapidPrefix + value.mapId]){
                    _this.areaMapWater[_this.mapidPrefix + value.mapId].attr({
                        display:'block'
                    })
                    _this.waterText[_this.mapidPrefix + value.mapId].attr({
